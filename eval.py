...
def save_video(frames_uint8: np.ndarray, output_path: str, fps: int = 8):
    """Save uint8 frames (T, H, W, C) as mp4."""
    try:
...
def main():
    ...
    parser.add_argument("--resolution",   type=int, default=None,
                        help="Override eval resolution (must be divisible by patch_size)")
    ...
    if args.resolution is not None:
        assert args.resolution % model_cfg.patch_size == 0, (
            f"--resolution {args.resolution} must be divisible by "
            f"patch_size {model_cfg.patch_size}"
        )
        old_res              = model_cfg.resolution
        model_cfg.resolution = args.resolution
        old_np, new_np       = (old_res // model_cfg.patch_size) ** 2, model_cfg.n_patches
        print(
    ...
    print(f"Generated {total_gen} frames, shape={generated.shape}")
    ...
    if args.output is None:
        args.output = f"eval_ep{args.episode}_{Path(args.checkpoint).stem}_steps{args.n_steps}.mp4"
    ...
    if args.with_gt and gt_uint8 is not None:
        ...
        save_video(np.concatenate([context_uint8, generated_uint8], axis=0), args.output, fps=args.fps)
